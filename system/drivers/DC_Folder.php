...
<input type="submit" name="saveNclose" id="saveNclose" class="tl_submit" alt="save all changes and return" accesskey="c" value="'.specialchars($GLOBALS['TL_LANG']['MSC']['saveNclose']).'" />
	...
	protected function generateTree($path, $intMargin, $mount=false, $blnProtected=false)
	{
		...
		for ($f=0; $f<count($folders); $f++)
		{
			...
			$return .= $this->generateImage($folderImg, '', $folderAttribute).' <strong>'.specialchars(basename($currentFolder)).'</strong></div> <div class="tl_right">';
		...
		for ($h=0; $h<count($files); $h++)
		{
			...
			{
				$return .= '<a href="typolight/popup.php?src='.$currentEncoded.'" title="'.specialchars($GLOBALS['TL_LANG']['MSC']['view']).'" onclick="this.blur(); Backend.openWindow(this, '.$popupWidth.', '.$popupHeight.'); return false;" >' . $this->generateImage($objFile->icon).' '.utf8_convert_encoding(specialchars(basename($currentFile)), $GLOBALS['TL_CONFIG']['characterSet']).'</a>'.$thumbnail.'</div> <div class="tl_right">';
			}
